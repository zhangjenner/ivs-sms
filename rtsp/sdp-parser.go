...
// ConvertSDP - SDP数据解析
func ConvertSDP(sdpRaw, url string) string {
	srcURL := ""
	sdpMap := ParseSDP(sdpRaw)
	for _, field := range [2]string{"audio", "video"} {
		if sdp, ok := sdpMap[field]; ok {
			if strings.Contains(sdp.Control, "/") {
				srcURL = sdp.Control[:strings.LastIndex(sdp.Control, "/")]
			}
			sdpRaw = strings.ReplaceAll(sdpRaw, sdp.Control, GetCtl(sdp.Control))
		}
	}
	if srcURL != "" {
		sdpRaw = strings.ReplaceAll(sdpRaw, srcURL, url)
	}
	return sdpRaw
}

// GetCtl - 获取控制标志
func GetCtl(control string) string {
	if strings.Contains(control, "/") {
		return control[strings.LastIndex(control, "/")+1:]
	}
	return control
}
